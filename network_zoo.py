...
def shallownet(nb_classes):
    global img_size
    model = Sequential()
    model.add(Convolution2D(64, kernel_size=(5, 5), padding='same', input_shape=img_size))
    model.add(BatchNormalization(axis=1))
    model.add(Activation('relu'))
...
def deepnet(nb_classes):
    global img_size
    model = Sequential()
    model.add(Convolution2D(64, kernel_size=(3, 3), padding='same', input_shape=img_size))
    model.add(BatchNormalization(axis=1))
